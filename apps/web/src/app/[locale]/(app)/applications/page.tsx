...
/* ─── Status Config ─────────────────────────────────────────────────── */
const getStatusSteps = (t: any) => [t('application_received'), t('under_review'), t('interviewing'), t('offer_extended')];
...
/* ─── Application Card ──────────────────────────────────────────────── */
function ApplicationCard({ job, t }: { job: MockJob, t: any }) {
  ...
  return (
    <div className="bg-white dark:bg-surface border border-[#E8EAF0] dark:border-border rounded-[20px] p-6 md:p-8 hover:border-primary/30 hover:shadow-[0_4px_24px_rgba(0,122,255,0.07)] transition-all duration-300 group">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[13px] font-semibold text-text-secondary mb-7">
        ...
        <div className="flex items-center gap-1.5">
          <Clock size={13} />
          {t('applied_on')} {new Date().toLocaleDateString()}
        </div>
      </div>
...
/* ─── Stats Bar ─────────────────────────────────────────────────────── */
function StatsBar({ jobs, t }: { jobs: MockJob[], t: any }) {
  const counts = [0, 1, 2, 3].map(i => jobs.filter(j => getStatusIndex(j) === i).length);
  const labels = [t('application_received'), t('under_review'), t('interviewing'), t('offer_extended')];
  ...
  return (
    <div className="flex flex-col min-h-full max-w-4xl mx-auto w-full px-4 md:px-8 py-8 md:py-12">
        ...
        <div className="flex items-center gap-4 mb-3">
          ...
          <div>
            ...
            <p className="text-[15px] text-text-secondary font-medium mt-0.5">
              {appliedJobs.length > 0 ? t('subtitle') : t('subtitle')}
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        /* ── Applications List ────────────────────────────────────── */
        <>
          <StatsBar jobs={appliedJobs} t={t} />
          <div className="flex flex-col gap-4">
            {appliedJobs.map(job => (
              <ApplicationCard key={job.id} job={job} t={t} />
            ))}
          </div>
        </>
