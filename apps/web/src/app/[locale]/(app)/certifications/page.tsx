...
export default function CertificationsPage() {
      ...
      <div className="flex flex-col min-h-full">
        ...
        <div className="flex-1 px-5 py-6 max-w-2xl">
          <div className="bg-surface p-8 rounded-[20px] border border-border text-center">
            <p className="text-text-secondary text-[15px] leading-relaxed">{t('set_trade')}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col min-h-full">
      <PageHeader
        title={t('title')}
        subtitle={`${t('required_for')} ${trade.charAt(0).toUpperCase() + trade.slice(1)}`}
      />
      <main className="flex-1 px-5 py-6 max-w-2xl w-full">

      {certifications.length === 0 ? (
        <div className="bg-surface p-8 rounded-[20px] border border-border text-center">
          <p className="text-text-secondary">{t('no_certs')}</p>
        </div>
      ) : (
