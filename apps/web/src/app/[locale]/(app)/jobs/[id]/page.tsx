...
export default function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const t = useTranslations('job_detail');
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-start">
        ...
        <div className="flex-1 w-full space-y-10">
          ...
          <div className="bg-white dark:bg-surface border border-[#E8EAF0] dark:border-border rounded-2xl p-8 space-y-10">
            {/* About */}
            <section>
              <h2 className="text-[20px] font-extrabold text-[#1D1D1F] dark:text-text-primary mb-4">{t('about_role')}</h2>
            ...
            {/* Required Skills */}
            <section>
              <h2 className="text-[20px] font-extrabold text-[#1D1D1F] dark:text-text-primary mb-4">{t('required_skills')}</h2>
            ...
            {job.requiredCerts.length > 0 && (
              <section>
                <h2 className="text-[20px] font-extrabold text-[#1D1D1F] dark:text-text-primary mb-4">{t('required_certs')}</h2>
          ...
          <div className="bg-white dark:bg-surface border border-[#E8EAF0] dark:border-border rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className={`px-6 py-4 flex items-center justify-center gap-2 ${badge.bg} border-b ${badge.border}`}>
              ...
              <span className={`text-[15px] font-extrabold ${badge.text}`}>
                {t('match_profile', { score: job.matchScore })}
              </span>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-5 mb-8">
                <div className="flex gap-4">
                  ...
                  <div>
                    <p className="text-[13px] font-bold text-[#6B7280] dark:text-text-secondary uppercase tracking-wide">{t('location')}</p>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <p className="text-[13px] font-bold text-[#6B7280] dark:text-text-secondary uppercase tracking-wide">{t('job_type')}</p>
                ...
                {job.salary && (
                  <div className="flex gap-4">
                    ...
                    <div>
                      <p className="text-[13px] font-bold text-[#6B7280] dark:text-text-secondary uppercase tracking-wide">{t('salary')}</p>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <p className="text-[13px] font-bold text-[#6B7280] dark:text-text-secondary uppercase tracking-wide">{t('date_posted')}</p>
                    <p className="text-[15px] font-semibold text-[#1D1D1F] dark:text-text-primary mt-0.5">{formatPostedDate(job.postedAt, tCommon)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <button onClick={toggleSaved} className="w-full py-4 rounded-xl bg-[#F7F8FA] dark:bg-background border border-[#E8EAF0] dark:border-border text-[#1D1D1F] dark:text-text-primary font-bold text-[15px] hover:border-primary/40 transition-colors lg:hidden">
