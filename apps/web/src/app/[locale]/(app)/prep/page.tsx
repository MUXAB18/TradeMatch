...
export default function PrepPage() {
  ...
  const TAB_CONFIG: { id: Mode; label: string; icon: React.ReactNode; desc: string }[] = [
    { id: 'flashcard', label: t('tab_flashcards'), icon: <BookOpen size={16} />, desc: 'Flip-card practice' },
    { id: 'text',      label: t('tab_text_chat'),  icon: <MessageSquare size={16} />, desc: 'Type your answers' },
    { id: 'voice',     label: t('tab_voice'),      icon: <Mic size={16} />,           desc: 'Speak out loud' },
  ];
  ...
  const handleGenerateCards = async () => {
    ...
    try {
      const res  = await fetch('/api/prep/generate', {
        ...
        body: JSON.stringify({ trade: profile.trade, skills: profile.skills, certifications: profile.certifications, experienceLevel: profile.yearsExperience ? `${profile.yearsExperience} years` : 'Mid-level', locale }),
      });
  ...
  const startTextChat = async () => {
    ...
    try {
      const res  = await fetch('/api/prep/question', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ trade: profile.trade, skills: profile.skills, locale }),
      });
      const data = await res.json();
  ...
  const handleSendText = async () => {
    ...
    try {
      const res  = await fetch('/api/prep/feedback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ question: currentQuestion, answer: userText, trade: profile?.trade, locale }),
      });
      const data = await res.json();
  ...
  const submitVoiceAnswer = async () => {
    ...
    try {
      const res  = await fetch('/api/prep/feedback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ question: voiceQuestion, answer: transcript, trade: profile?.trade, locale }),
      });
      const data = await res.json();
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="bg-surface border-b border-border px-6 py-5 shrink-0">
        <h1 className="text-[22px] font-extrabold text-text-primary tracking-tight">{t('title')}</h1>
        <p className="text-[14px] text-text-secondary mt-0.5">{tradeName} {t('practice_questions')}</p>
        ...
        <div className="flex-1 px-5 py-6 max-w-2xl w-full mx-auto space-y-7">
          ...
          {prepCards.length === 0 && !isGenerating && (
            <div className="bg-white border-2 border-dashed border-[#D1D5DB] rounded-[24px] p-10 flex flex-col items-center justify-center text-center">
              ...
              <h2 className="text-[20px] font-extrabold text-[#1D1D1F] tracking-tight mb-2">{t('generate_flashcards')}</h2>
              ...
              >
                <Sparkles size={18} />
                {profile?.trade ? t('generate_for_trade', { trade: tradeName }) : t('add_trade_first')}
              </button>
            </div>
          )}
        ...
        <div className="flex-1 flex flex-col max-w-2xl w-full mx-auto px-5 py-6 min-h-0">
          ...
          {chatMessages.length === 0 && !chatLoading && (
            <div className="flex-1 flex flex-col items-center justify-center text-center gap-5">
              ...
              <div>
                <h2 className="text-[20px] font-extrabold text-text-primary mb-1">{t('text_interview_practice')}</h2>
                <p className="text-[14px] text-text-secondary w-full max-w-[360px] leading-relaxed">
                  {t('text_interview_desc')}
                </p>
              </div>
              <button
              ...
              >
                <ChevronRight size={18} /> {t('start_practice_session')}
              </button>
            </div>
          )}
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pb-4">
                {chatMessages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
                    {msg.role === 'ai' && (
                      <div className="w-8 h-8 rounded-full bg-[#007AFF] flex items-center justify-center me-2 shrink-0 mt-1">
                    ...
                    <div
                      className={`max-w-[80%] px-4 py-3 rounded-2xl text-[14px] leading-relaxed ${
                        msg.role === 'user'
                          ? 'bg-[#007AFF] text-white rounded-se-sm'
                          : msg.feedback
                            ? 'bg-[#F0FDF4] border border-[#BBF7D0] text-[#166534] rounded-ss-sm'
                            : 'bg-surface border border-border text-text-primary rounded-ss-sm'
                      }`}
                    >
                      {msg.text}
                    </div>
                ...
                {chatLoading && (
                  <div className="flex items-start gap-2">
                    ...
                    <div className="bg-surface border border-border rounded-2xl rounded-ss-sm px-4 py-3">
        ...
        <div className="flex-1 flex flex-col items-center justify-center max-w-2xl w-full mx-auto px-5 py-6 gap-6">
          ...
          {!voiceQuestion && !voiceLoading && (
            <div className="text-center space-y-5">
              ...
              <div>
                <h2 className="text-[20px] font-extrabold text-text-primary mb-1">{t('voice_interview_practice')}</h2>
                <p className="text-[14px] text-text-secondary w-full max-w-[360px] mx-auto leading-relaxed">
                  {t('voice_interview_desc')}
                </p>
              </div>
              <button
              ...
              >
                <Volume2 size={18} /> {t('start_voice_session')}
              </button>
            </div>
          )}
