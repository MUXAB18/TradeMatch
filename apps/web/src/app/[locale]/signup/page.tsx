    ...
    return (
      <div className="grid min-h-screen w-full lg:grid-cols-[45%_55%] xl:grid-cols-2">
        ...
        <div className="hidden lg:flex relative bg-gradient-to-br from-[#0055FF] to-[#007AFF] flex-col justify-between p-12 overflow-hidden">
          <div className="absolute top-0 end-0 w-[800px] h-[800px] bg-white/5 rounded-full blur-3xl translate-x-1/3 -translate-y-1/3" />
          <div className="absolute bottom-0 start-0 w-[600px] h-[600px] bg-white/10 rounded-full blur-3xl -translate-x-1/4 translate-y-1/4" />
          ...
          <div className="relative z-10">
            <h1 className="text-5xl font-extrabold text-white leading-tight mb-4 tracking-tight">
              {tSignup('hero_title_1')}<br />{tSignup('hero_title_2')}<br /><span className="text-white/80">{tSignup('hero_title_3')}</span>
            </h1>
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            <p className="text-white/90 text-sm font-medium">{tSignup('trusted_by')}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col justify-center items-center p-6 sm:p-10 bg-gray-50/50">
          <div className="w-full max-w-[440px]">
            ...
            <p className="text-[15px] text-gray-500 mb-8">{tSignup('subtitle_role')}</p>

            <div className="space-y-4">
              {/* Worker card */}
              <button
                onClick={() => handleSelectRole('worker')}
                className="w-full group flex items-start gap-4 p-5 bg-white border-2 border-gray-200 rounded-2xl hover:border-[#007AFF] hover:shadow-md transition-all duration-200 text-start active:scale-[0.99]"
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h3 className="text-[18px] font-extrabold text-gray-900">{tSignup('worker_title')}</h3>
              ...
              <button
                ...
                className="w-full group flex items-start gap-4 p-5 bg-white border-2 border-gray-200 rounded-2xl hover:border-[#8B5CF6] hover:shadow-md transition-all duration-200 text-start active:scale-[0.99]"
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h3 className="text-[18px] font-extrabold text-gray-900">{tSignup('agency_title')}</h3>
            ...
            <div className="flex justify-center items-center mt-8">
              <span className="text-[15px] text-gray-500 me-2">{tSignup('already_account')}</span>
              <Link href="/login" className="text-[15px] font-bold text-[#007AFF] hover:opacity-80">{tAuth('login_link')}</Link>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="flex min-h-screen w-full bg-white">
      ...
      <div className={`hidden lg:flex w-[45%] relative flex-col justify-between p-12 overflow-hidden ${role === 'agency' ? 'bg-gradient-to-br from-[#4C1D95] to-[#8B5CF6]' : 'bg-gradient-to-br from-[#0055FF] to-[#007AFF]'}`}>
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-5xl font-extrabold text-white leading-tight mb-4 tracking-tight whitespace-pre-line">
            {role === 'agency' ? tSignup('agency_hero_title') : tSignup('worker_hero_title')}
          </h1>
          <p className="text-white/80 text-lg leading-relaxed">
            {role === 'agency'
              ? tSignup('agency_hero_desc')
              : tSignup('worker_hero_desc')}
          </p>
        </div>
        <button onClick={() => setStep('role')} className="relative z-10 self-start text-white/70 text-sm hover:text-white transition-colors">
          ← {tSignup('change_account_type')}
        </button>
      </div>
      ...
      <div className="w-full lg:w-[55%] flex flex-col justify-center items-center p-6 sm:p-10 bg-gray-50/50">
        <div className="w-full max-w-[420px]">
          ...
          <button onClick={() => setStep('role')} className="lg:hidden mb-4 text-sm text-gray-500 hover:text-gray-700">← {tSignup('back')}</button>
          ...
          <h2 className="text-[26px] font-extrabold text-gray-900 tracking-tight mb-1">{tSignup('create_account')}</h2>
          <p className="text-sm text-gray-500 mb-6">
            {authMethod === 'email' ? tSignup('fill_details') : tSignup('phone_prompt')}
          </p>
          ...
          {authMethod === 'email' ? (
            <form onSubmit={handleSubmit} className="flex flex-col gap-3 mb-6">
              ...
              {[
                { label: tAuth('name'), value: name, setter: setName, type: 'text', placeholder: 'Your full name', autoComplete: 'name' },
                { label: tAuth('email'), value: email, setter: setEmail, type: 'email', placeholder: 'you@example.com', autoComplete: 'email' },
                { label: tAuth('password'), value: password, setter: setPassword, type: 'password', placeholder: '••••••••', autoComplete: 'new-password' },
              ].map(field => (
            ...
            <form onSubmit={handleSendCode} className="flex flex-col gap-3 mb-6">
              ...
              <div className="relative h-14 border border-gray-200 bg-white shadow-sm rounded-xl px-4 flex flex-col justify-center focus-within:border-[#007AFF] focus-within:ring-4 focus-within:ring-[#007AFF]/10 transition-all">
                <label className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">{tSignup('phone_number')}</label>
                <div className="flex items-center w-full mt-0.5">
                  <div className="me-2"><CountryCodeSelect value={countryCode} onChange={setCountryCode} /></div>
                  <input
                    type="tel"
                    value={phoneNumber}
              ...
              >
                {loading ? tSignup('btn_sending') : tSignup('btn_send_code')}
              </button>
            </form>
          )}
          ...
          <div className="relative flex items-center justify-center mb-5">
            ...
            <span className="relative bg-gray-50/50 px-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">{tSignup('or_continue_with')}</span>
          </div>
